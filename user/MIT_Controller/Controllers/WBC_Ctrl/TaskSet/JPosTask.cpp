...
JPosTask<T>::JPosTask(const FloatingBaseModel<T>* robot)
    : Task<T>(cheetah::max_num_act_joint), robot_sys_(robot) {
  TK::Jt_ = DMat<T>::Zero(cheetah::max_num_act_joint, cheetah::max_dim_config);
  (TK::Jt_.block(0, 6, cheetah::max_num_act_joint, cheetah::max_num_act_joint))
      .setIdentity();
  TK::JtDotQdot_ = DVec<T>::Zero(cheetah::max_num_act_joint);

  _Kp = DVec<T>::Constant(cheetah::max_num_act_joint, 50.);
  _Kd = DVec<T>::Constant(cheetah::max_num_act_joint, 5.);
}
